...
-- run via :terminal, not :! — a pty merges the program's stdout and stderr
-- at the kernel level (true program order, no scrambling across nvim's
-- separate pipes) and renders ANSI colors: colored debug(), gcc/sanitizer
-- diagnostics, cargo output
local function setup_cpp()
  local exec = run_input .. './%:r.out < input'
  local function compile_run(flags)
    return '<CMD>w!<CR><CMD>terminal g++ ' .. flags .. ' %:r.cpp -o %:r.out && ' .. exec .. '<CR>'
  end
  ...
  map('<F10>', '<CMD>terminal ' .. exec .. '<CR>', 'CP: run')
end
...
local function setup_rust()
  ...
  if cargo_toml then
    ...
    map('<F4>', '<CMD>w!<CR><CMD>terminal ' .. run_input .. 'cargo run --release < input<CR>', 'CP: cargo run --release')
    map('<F5>', '<CMD>w!<CR><CMD>terminal ' .. run_input .. 'cargo run < input<CR>', 'CP: cargo run')
    -- debug builds already have overflow & bounds checks
    map('<F6>', '<CMD>w!<CR><CMD>terminal ' .. run_input .. 'RUST_BACKTRACE=1 cargo run < input<CR>', 'CP: cargo run + backtrace')
    ...
    map('<F10>', '<CMD>terminal ' .. run_input .. './target/debug/' .. bin .. ' < input<CR>', 'CP: run')
  else
    ...
    local exec = run_input .. './%:r.out < input'
    local function compile_run(flags)
      return '<CMD>w!<CR><CMD>terminal rustc --edition 2024 ' .. flags .. ' %:r.rs -o %:r.out && ' .. exec .. '<CR>'
    end
...
local function setup_python()
  map('<F5>', '<CMD>w!<CR><CMD>terminal ' .. run_input .. 'pypy3 %:r.py < input<CR>', 'CP: run with pypy3')
end
...
local function setup_kotlin()
  ...
  map('<F4>', '<CMD>w!<CR><CMD>terminal kotlinc %:r.kt && ' .. exec .. '<CR>', 'CP: compile + run')
  map('<F5>', '<CMD>w!<CR><CMD>terminal kotlinc %:r.kt && ' .. exec .. '<CR>', 'CP: compile + run')
  map('<F6>', '<CMD>w!<CR><CMD>terminal kotlinc %:r.kt && ' .. run_input .. 'kotlin -J-ea %:rKt.class < input<CR>', 'CP: compile + run with assertions')
